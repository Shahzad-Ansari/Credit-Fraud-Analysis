...
test_notFraud <- test[test$is_fraud == 0,]

train_fraud <- train[train$is_fraud == 1,]
train_notFraud <- train[train$is_fraud == 0,] 

test = rbind(head(test_notFraud,n = 450),head(test_fraud,n=50))
train = rbind(head(train_notFraud,n = 1250),head(train_fraud,n=250))
...
# to do RBIND need to fix data type mismatch

test$trans_date_trans_time = as.factor(test$trans_date_trans_time)
test$dob = as.factor(test$dob)
...
NNTrain = dplyr::select(train, c(is_fraud, merch_lat , 
                                   merch_long , lat , long , amt))

NNTest = dplyr::select(test, c(is_fraud, merch_lat , 
                                merch_long , lat , long , amt))

dim(NNTest)
nnMatrix = model.matrix(~., data = NNTrain)


nn1 = neuralnet(is_fraud ~ merch_lat + 
                  merch_long + lat + long + amt , 
                data = nnMatrix,
                lifesign = "full",
                hidden =  c(5,3),
...
pred = compute(nn1,NNTest) # To predict on test set
class_pred = round(pred$net.result)


class_pred

differences = ifelse(class_pred==NNTest$is_fraud,0,1)

percentage = sum(differences)/dim(differences)[1]
...
resultdf = as.data.frame(class_pred)



names(resultdf)[1] = "Prediction"
resultdf[,"Test"] =  NNTest$is_fraud
resultdf[,'dif'] = differences
